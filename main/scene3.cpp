...
int main() {
  ...
  /* Set Point_light */
  world.lights.push_back(new Point_Light(vec3(0,0,0),white,20));


  /* TODO: Set Objects */
  world.object_list.push_back(new Plane(vec3(-1, -1, 0), vec3(0,1,0), shader_gray));
  world.object_list.push_back(new Plane(vec3(-1, -1, 0), vec3(1,0,0), shader_red));
  
  world.Render();
  end=clock();
  std::cout<<"Running Time: "<<(double)(end-start)/CLOCKS_PER_SEC<<" s"<<std::endl;
  /* TODO: Save Image */
  Dump_png(world.camera.colors, width, height, "./result/scene3.jpg");
