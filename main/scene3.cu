...
int main(int argc, char *argv[]) {
    ...
    createLight<<<1,1>>>(lights, 0, vec3(0,0,0), white, 20); // TODO: parse from file 
    ...
    addPlane<<<1,1>>>(objs, 0, shaders, 4, vec3(-1,-1,0), vec3(0,1,0)); 
    addPlane<<<1,1>>>(objs, 1, shaders, 0, vec3(-1,-1,0), vec3(1,0,0)); 
    checkCudaErrors(cudaDeviceSynchronize());
    ...
    // Save img
    Dump_png(colors, width, height, "./result/scene3.png");
