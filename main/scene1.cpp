...
int main() {
  ...
  /* Set Point_light */
  world.lights.push_back(new Point_Light(vec3(0,4,6),white,200));
  world.lights.push_back(new Point_Light(vec3(-3,1,6),magenta,200));


  /* TODO: Set Objects */
  world.object_list.push_back(new Sphere(vec3(0, 0, 0), 0.5, shader_white));
  world.object_list.push_back(new Sphere(vec3(1, 0, 0), 0.5, shader_red));
  world.object_list.push_back(new Sphere(vec3(0, 1, 0), 0.5, shader_green));
  world.object_list.push_back(new Sphere(vec3(0, 0, 1), 0.5, shader_blue));
  world.object_list.push_back(new Plane(vec3(0, -1, 0), vec3(0,1,0), shader_gray));
  
  world.Render();
  end=clock();
  std::cout<<"Running Time: "<<(double)(end-start)/CLOCKS_PER_SEC<<" s"<<std::endl;
  /* TODO: Save Image */
  Dump_png(world.camera.colors, width, height, "./result/scene1.jpg");
