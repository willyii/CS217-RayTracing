...
int main(int argc, char *argv[]) {
    ...
    createLight<<<1,1>>>(lights, 0, vec3(0,5,6), white, 300); // TODO: parse from file 
    createLight<<<1,1>>>(lights, 1, vec3(-4,2,6), white, 300); // TODO: parse from file 
    createLight<<<1,1>>>(lights, 2, vec3(0,-3,6), green, 10); // TODO: parse from file 
    ...
    addSphere<<<1,1>>>(objs, 1, shaders, 2, vec3(0,0,1), 0.5); 
    addPlane<<<1,1>>>(objs, 2, shaders, 4, vec3(0,-2,0), vec3(0,1,0)); 
    checkCudaErrors(cudaDeviceSynchronize());
    ...
    // Save img
    Dump_png(colors, width, height, "./result/scene2.png");
